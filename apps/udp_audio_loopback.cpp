...
ProgramOptions parse_options(int argc, char** argv) {
  ...
  if (argc > 7) {
    ...
    if (mode == "none" || mode == "repeat" || mode == "periodic" || mode == "periodic_interp") {
...
void play_expected_frame(std::uint32_t sequence,
  ...
  if (!frame.has_value()) {
    ...
    ++stats.concealed;
    const bool use_periodic_plc =
      plc_mode == PlcMode::periodic || plc_mode == PlcMode::periodic_interp;
    smooth_boundary_if_needed(concealed_frame, smoother, !use_periodic_plc);
    if (recorder != nullptr) {
  ...
  const bool use_periodic_plc =
    plc_mode == PlcMode::periodic || plc_mode == PlcMode::periodic_interp;
  smooth_boundary_if_needed(output_frame, smoother, false,
                            !use_periodic_plc && smoother.previous_was_concealed);
  if (recorder != nullptr) {
